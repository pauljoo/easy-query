 ...
 */
public class MongoProject extends Project implements info.demo.dsl.server.mongodb.MongoRel {
    public MongoProject(RelOptCluster cluster, RelTraitSet traitSet,
                        RelNode input, List<? extends RexNode> projects, RelDataType rowType) {
        super(cluster, traitSet, ImmutableList.of(), input, projects, rowType);
        assert getConvention() == info.demo.dsl.server.mongodb.MongoRel.CONVENTION;
    ...
    public void implement(Implementor implementor) {
        ...
        final info.demo.dsl.server.mongodb.MongoRules.RexToMongoTranslator translator =
                new info.demo.dsl.server.mongodb.MongoRules.RexToMongoTranslator(
        ...
        for (Pair<RexNode, String> pair : getNamedProjects()) {
            ...
            items.add(expr.equals("'$" + name + "'")
                    ? info.demo.dsl.server.mongodb.MongoRules.maybeQuote(name) + ": 1"
                    : info.demo.dsl.server.mongodb.MongoRules.maybeQuote(name) + ": " + expr);
        }
