...
public class MongoAggregate
    ...
    public void implement(Implementor implementor) {
        ...
        final List<String> inNames =
                info.xpanda.dsl.server.mongodb.MongoRules.mongoFieldNames(getInput().getRowType());
        final List<String> outNames = info.xpanda.dsl.server.mongodb.MongoRules.mongoFieldNames(getRowType());
        int i = 0;
        if (groupSet.cardinality() == 1) {
            final String inName = inNames.get(groupSet.nth(0));
            list.add("_id: " + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote("$" + inName));
            ++i;
        } else {
            List<String> keys = new ArrayList<>();
            for (int group : groupSet) {
                final String inName = inNames.get(group);
                keys.add(inName + ": " + info.xpanda.dsl.server.mongodb.MongoRules.quote("$" + inName));
                ++i;
            }
        ...
        for (AggregateCall aggCall : aggCalls) {
            list.add(
                    info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote(outNames.get(i++)) + ": "
        ...
        if (groupSet.cardinality() == 1) {
            fixups = new AbstractList<String>() {
                ...
                public String get(int index) {
                    ...
                    return info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote(outName) + ": "
                            + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote("$" + (index == 0 ? "_id" : outName));
                }
            ...
            for (int group : groupSet) {
                fixups.add(
                        info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote(outNames.get(group))
                                + ": "
                                + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote("$_id." + outNames.get(group)));
                ++i;
            }
            for (AggregateCall ignored : aggCalls) {
                final String outName = outNames.get(i++);
                fixups.add(
                        info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote(outName) + ": " + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote(
                                "$" + outName));
            }
        }
        if (!groupSet.isEmpty()) {
    ...
    private static String toMongo(SqlAggFunction aggregation, List<String> inNames,
        ...
        if (aggregation == SqlStdOperatorTable.COUNT) {
            if (args.size() == 0) {
                ...
                return "{$sum: {$cond: [ {$eq: ["
                        + info.xpanda.dsl.server.mongodb.MongoRules.quote(inName)
                        + ", null]}, 0, 1]}}";
            }
            ...
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$sum: " + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote("$" + inName) + "}";
        } else if (aggregation == SqlStdOperatorTable.MIN) {
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$min: " + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote("$" + inName) + "}";
        } else if (aggregation == SqlStdOperatorTable.MAX) {
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$max: " + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote("$" + inName) + "}";
        } else if (aggregation == SqlStdOperatorTable.AVG) {
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$avg: " + info.xpanda.dsl.server.mongodb.MongoRules.maybeQuote("$" + inName) + "}";
        } else {
